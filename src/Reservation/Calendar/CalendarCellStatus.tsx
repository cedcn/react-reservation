...
const CalendarCellStatus: React.FC<CellStatusProps> = (props) => {
  const { isSelectable, isSelected, isMakefull, remaining, remainingMaxThreshold, current } = props
  ...
  return (
    <div className="reservation-cell__status" css={styles.cellStatus}>
      <span css={styles.cellStatusLabel}>{current.format('DD')}</span>
      {!!content && <div css={styles.cellStatusContent}>{content}</div>}
    </div>
  )
}

export default CalendarCellStatus
